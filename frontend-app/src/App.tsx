...
export default function App() {
  ...
  const [hasEndTimeSelected, setHasEndTimeSelected] = useState<boolean | null>(null);
  const sharedResultLoaded = useRef(false);

  const currentQuestion = questions[step];
  const currentValue = answers[currentQuestion.key];
  const canContinue = currentQuestion.key === "hasEndTime" ? hasEndTimeSelected !== null : Boolean(currentValue);
  ...
  function selectAnswer(value: string) {
    ...
    if (key === "hasEndTime") {
      setHasEndTimeSelected(value === "true");
    }
    setAnswers((current) => ({
      ...current,
  ...
  async function submitAnalysis(requestAnswers = answers) {
    ...
    try {
      ...
      setAnalysis(result);
      setPhase("result");
      replaceSearch(searchFromAnswers(requestAnswers));
    } catch {
  ...
  useEffect(() => {
    ...
    setHasEndTimeSelected(sharedAnswers.hasEndTime);
